package com.csonezp.graph.dijkstra;

/**
 * @author zhangpeng
 * @date 2019-06-20 13:27
 */
/**
...
public class MatrixUDG {
     ...
     */
    public int[][] floyd(int[][] path, int[][] dist) {
        ...
        for (int i = 0; i < mVexs.length; i++) {
            for (int j = 0; j < mVexs.length; j++)
                System.out.printf("%2s  ", vexs[path[i][j]]);
            System.out.printf("\n");
        }
        return path;
    ...
    public static void main(String[] args) {
        ...
        int[][] pathRes = pG.floyd(path, floy);
        for (int i = 0; i < pG.mVexs.length/2; i++) {
            for (int j = 0; j < pG.mVexs.length; j++) {
                if(i!=j){
                    System.out.println(vexs[i] + "到 "+vexs[j]+"的最短路径：");
                    System.out.print(vexs[i]);
                    printPath(pathRes,i,j);
                }
